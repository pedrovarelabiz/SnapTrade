...
export function masanielloRecordTrade(
  snapshot: MasanielloSnapshot,
  isWin: boolean,
  actualStake?: number,
): MasanielloSnapshot {
  const dayTrades = snapshot.dayTrades + 1;
  const dayWins = snapshot.dayWins + (isWin ? 1 : 0);
  const stakeUsed = actualStake ?? masanielloGetStake(snapshot);
  const targetProfit = isWin
    ? Math.max(0, snapshot.targetProfit - stakeUsed * snapshot.payoutRate)
