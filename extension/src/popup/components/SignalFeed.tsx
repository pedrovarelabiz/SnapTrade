...
type TradeSource = 'indicators' | 'ai' | 'backend' | 'telegram';
...
const SOURCE_STYLES: Record<TradeSource, { label: string; bg: string }> = {
  indicators: { label: 'IND', bg: '#00e676' },
  ai:         { label: 'AI',  bg: '#7c4dff' },
  backend:    { label: 'BE',  bg: '#2979ff' },
  telegram:   { label: 'TG',  bg: '#ff9100' },
};

function getSourceFromId(signalId: string, channelSlug?: string): TradeSource {
  ...
  // ws_ and bal_ are internally-generated IDs from WS result correlation failure
  // Since SINGLE PATH architecture: all trades go through AI batch, so ws_ = AI trade with lost correlation
  if (channelSlug === 'ai_analyst') return 'ai';
  if (channelSlug === 'st_indicators') return 'indicators';
  if (signalId.startsWith('ws_') || signalId.startsWith('bal_')) return 'ai';
  return 'telegram';
}
...
const SignalFeedItem: React.FC<SignalFeedItemProps> = ({
  ...
  const srcStyle = SOURCE_STYLES[item.source];
    ...
    <div className="signal-item">
      ...
      <span
        style={{
          ...
          background: srcStyle.bg, borderRadius: '3px',
          padding: '1px 4px', marginRight: '4px', lineHeight: '14px',
        }}
      >
        {srcStyle.label}
      </span>
      ...
      {item.confidence !== null && (
        <span style={{
          fontSize: '9px', fontWeight: 700, borderRadius: '3px', padding: '1px 3px', marginRight: '2px',
          color: '#fff',
          background: item.confidence >= 80 ? '#00c853' : item.confidence >= 60 ? '#ffc107' : '#ff5252',
        }}>
          {item.confidence}%
        </span>
      )}
...
function buildFeedItems(
  ...
  const fromSignals: FeedItem[] = pendingSignals.map((s) => ({
    ...
    signalId: s.id,
    source: getSourceFromId(s.id, s.channel?.slug),
  ...
  const fromTrades: FeedItem[] = recentTrades.slice(-10).map((t) => ({
    ...
    signalId: t.signalId,
    source: getSourceFromId(t.signalId),
    confidence: null, // Trades don't carry confidence
  }));
