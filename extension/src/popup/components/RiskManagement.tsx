    ...
    <div className="card" style={{ marginBottom: 8 }}>
      ...
      {open && (
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                onClick={() => {
                  if (settings.tradingMode !== 'real') {
                    const confirmed = window.confirm(
                      'REAL MONEY MODE\nTrades will use your real balance.\nMake sure you have tested in demo mode first.\nConfirm switch to REAL?'
                    );
                    if (!confirmed) return;
                  }
                  onUpdate({ tradingMode: 'real' });
                }}
                style={{ minWidth: 50 }}
              >
                REAL
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <label style={{ fontSize: 12, color: '#8b8b9e' }}>Max daily loss</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ fontSize: 12, color: '#8b8b9e' }}>$</span>
              <input type="number" className="input" style={{ width: 60, textAlign: 'center' }} value={settings.maxDailyLoss ?? 0} onChange={e => onUpdate({ maxDailyLoss: parseInt(e.target.value) || 0 })} placeholder="0" />
            </div>
          </div>
          <div style={{ fontSize: 10, color: '#8b8b9e', marginTop: -6 }}>Stop trading when daily P&L reaches -$X (0 = disabled)</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <label style={{ fontSize: 12, color: '#8b8b9e' }}>Min payout to trade</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <input type="number" className="input" style={{ width: 60, textAlign: 'center' }} value={settings.minPayoutPercent ?? 70} onChange={e => onUpdate({ minPayoutPercent: parseInt(e.target.value) || 0 })} min={0} max={100} placeholder="70" />
              <span style={{ fontSize: 12, color: '#8b8b9e' }}>%</span>
            </div>
          </div>
          <div style={{ fontSize: 10, color: '#8b8b9e', marginTop: -6 }}>Blocks trades on pairs with payout below this (0 = disabled)</div>
