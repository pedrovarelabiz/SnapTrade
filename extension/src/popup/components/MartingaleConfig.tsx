...
export function MartingaleConfig({ settings, onUpdate }: Props) {
  ...
  const strategies: Array<{ value: ExtensionSettings['strategy']; label: string; desc: string }> = [
    ...
    { value: 'sorosgale', label: 'Sorosgale', desc: 'Soros on wins + Gale recovery on losses' },
    ...
    <div className="card" style={{ marginBottom: 8 }}>
      ...
      {open && (
        <div style={{ marginTop: 12 }}>
          ...
          {settings.strategy === 'sorosgale' && (
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <label style={{ fontSize: 12, color: '#8b8b9e' }}>Soros levels</label>
                <select className="input" style={{ width: 60 }} value={settings.sorosMaxLevels} onChange={e => onUpdate({ sorosMaxLevels: parseInt(e.target.value) })}>
                  {[2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
                </select>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <label style={{ fontSize: 12, color: '#8b8b9e' }}>Gale max levels</label>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <label style={{ fontSize: 12, color: '#8b8b9e' }}>Gale multiplier</label>
          ...
          {settings.strategy === 'simple' && (
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                <label style={{ fontSize: 11, color: '#8b8b9e' }}>Gale mode</label>
                <select className="input" style={{ width: 110, fontSize: 11 }} value={(settings as any).galeMode ?? 'per-signal'} onChange={e => onUpdate({ galeMode: e.target.value } as any)}>
                  <option value="per-signal">Per signal</option>
                  <option value="per-asset">Per asset</option>
                </select>
              </div>
              <div style={{ fontSize: 9, color: '#5a5a6e', marginTop: 2 }}>
                Per signal: next trade (any pair) uses gale. Per asset: retry same pair only.
              </div>
            </div>
          )}
        </div>
